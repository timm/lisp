// Extend Prism's lisp grammar to recognise plus macros and project sigils.
...
(function () {
  ...
  Prism.languages.insertBefore('lisp', 'keyword', {
    ...
    // Project macros. Trailing (?=[\s()]) handles `plus+`, `let+`, `for+`, `if+`, `new+`
    // where `+` is a non-word character and \b alone fails.
    'glu-macro': {
      pattern: /\b(?:plus\+?|let\+|for\+|if\+|new\+|defread|ff?f?\+)(?=[\s()])/,
      alias: 'keyword'
    },

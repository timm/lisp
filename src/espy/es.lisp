...
(defmethod combined ((c1 col) (c2 col))
  ...
  (let ((c3 (combine c1 c2))
        (n  (o c3 n)))
    (if (<= (div c3) (+ (* (div c1) (/ (o c1 n) n)) 
                        (* (div c2) (/ (o c2 n) n ))))
      c3)))
...
(defmethod var ((s sym)) "div of symbols is entropy" (entropy s))
...
(defmethod dist1 ((s sym) x y) 
  "same symbols are zero, else 1" 
  (if (eql x y) 0 1))

(defmethod combine ((s1 sym) (s2 sym))
  "combine two symbols"
  (let ((s3 (make-sym :txt (o s1 txt) (o s1 pos))))
    (loop for (x . n) in (o s1 seen) do (add s3 x n))
    (loop for (x . n) in (o s2 seen) do (add s3 x n))
    s3))
...
(defmethod mid ((n num)) "mid of numbers is median"         (per n .5))
(defmethod var ((n num)) "div of numbers is standard deviation" (sd n))
(defmethod sd  ((n num)) "sd"       (/ (- (per n .9) (per n .1)) 2.56))
...
(defmethod combine ((n1 num) (n2 num))
  "Combine numerics"
  (let ((n3 (make-num :txt (o n1 txt) (o n1 pos))))
    (loop for n across (o n1 _all) do (add n3 n))
    n3))
